\begin{solutionfigure}[htb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            % Comparator-consistent switching traces
            % carrier c(x)=4 + 0.5*(1 - 4*abs(mod(8x,1)-0.5))
            % reference s*(x)=4+0.5 sin(360x)
            \addplot[black, ultra thick, domain=0:1, samples=2400] {ifthenelse(4+0.5*sin(360*x) > 4 + 0.5*(1 - 4*abs(mod(8*x,1)-0.5)),2.95,2.25)};
            \addplot[black, ultra thick, domain=0:1, samples=2400] {ifthenelse(4+0.5*sin(360*x) > 4 + 0.5*(1 - 4*abs(mod(8*x,1)-0.5)),1.15,1.85)};
            \addplot[black, ultra thick, domain=0:1, samples=2400] {ifthenelse(4+0.5*sin(360*x) > 4 + 0.5*(1 - 4*abs(mod(8*x,1)-0.5)),0.7,-0.7)};
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Solution plot: complementary PWM and switching state over one electrical period ($\omega_1 t\in[0,2\pi]$).}
    \label{fig:task04_pwm_solution_cs}
\end{solutionfigure}
