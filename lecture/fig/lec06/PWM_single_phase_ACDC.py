...
# generate a fundamnetal and normalized sinusoidal reference d(t)
def d_sin(wt):
    return np.sin(wt)
...
s_int_example = s_int(d_int_example, c_int_example)
e_int_example = e(d_int_example, (s_int_example[0] - s_int_example[1])/2, xn)

# Compute the derivative of the signal
s_diff = np.diff((s_int_example[0] - s_int_example[1])/2)
...
np.savetxt(save_path, np.column_stack((x[idx_sum], s_int_example[0][idx_sum], s_int_example[1][idx_sum], (s_int_example[0][idx_sum] -s_int_example[1][idx_sum])/2, d_int_example[idx_sum], c_int_example[idx_sum], e_int_example[idx_sum])), delimiter=',', header='wt, s1, s2, s, d, c, e', comments='')
...
plt.plot(x, (s_int(d_sin(x), c_tri(x, N))[0] - s_int(d_sin(x), c_tri(x, N))[1])/2)
plt.xlabel(r'$\omega t$')
...
plt.plot(x, e(d_sin(x), (s_int(d_sin(x), c_tri(x, N))[0] - s_int(d_sin(x), c_tri(x, N))[1])/2, xn))
plt.xlabel(r'$\omega t$')
