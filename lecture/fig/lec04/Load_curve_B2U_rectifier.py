...
# save the data to a csv file with I_out being the first column, gain the second, alpha the third and beta the fourth to same folder as the script
current_directory = os.path.dirname(os.path.abspath(__file__))
save_path = os.path.join(current_directory, "Load_curve_B2U_rectifier.csv")
np.savetxt(
    save_path,
    np.column_stack((I_out, gain, alpha / np.pi, beta / np.pi)),
    delimiter=",",
    header="I_out, gain, alpha, beta",
    comments="",
)
