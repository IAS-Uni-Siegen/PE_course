            ...
            \begin{figure}
                \begin{tikzpicture}
                    \begin{axis}[
                        ...
                        \draw node[fill=white, inner sep = 1pt] at (axis cs:0.5,1) {linear mod.};
                        \draw node[fill=white, inner sep = 1pt] at (axis cs:2,1) {overmodulation};
                        \draw node[fill=white, inner sep = 1pt, above] at (axis cs:1.5,4/pi) {Square-shaped mod. ($f_\mathrm{s}=2\cdot\nicefrac{\omega}{2 \pi}$)};
                    \end{axis}
                \end{tikzpicture}
    ...
    \frametitle{Rectifier application setup} 
        \begin{figure}
            \begin{circuitikz}
                ...
                \draw (c1) to [capacitor, *-*, l=$C$] (c1 |- int2);
            ...
            \end{circuitikz}
            \caption{Single-phase grid rectification: full bidirectional operation possible (e.g., for electrical rail vehicles with a \SI{15}{\kilo\volt}, \SI[parse-numbers = false]{16\frac{2}{3}}{\hertz} grid)}
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{equation}
                \hat{u}_2 = \sqrt{\hat{u}^2_\mathrm{g} + \omega^2 L^2 \hat{i}^2_2}.
            \end{equation}
            As the converter boosts the grid voltage towards the DC-link, the following condition applies:
            \begin{equation}
                u_1(t)\approx U_\mathrm{dc} \geq \hat{u}_2 = \sqrt{\hat{u}^2_\mathrm{g} + \omega^2 L^2 \hat{i}^2_2}.
            \end{equation}
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{figure}
