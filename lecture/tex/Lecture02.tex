...
\begin{frame}
    ...
    We define the \hl{semiconductor utilization} as the ratio of the average output power to the transistor (peak) power:
    \begin{equation}
    ...
    \begin{itemize}
        ...
        \item The lower the semiconductor utilization, the more costly / bulky the transistor for a given converter power (key parameter for the selection of the power stage).
    \end{itemize}
    Assumptions for following calculations:
    \begin{itemize}
        \item Lossless operation in CCM,
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \end{equation}
            The transistor must block the (constant) input voltage $U_1$ and step-like changing current $i_1(t)=i_\mathrm{T}(t)$. The semiconductor utilization is
            \begin{equation}
                \begin{split}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{equation}
                ...
                \end{split}
            \end{equation}
            The transistor must block the (constant) output voltage $U_2$ and (constant) input current $i_1(t)=i_\mathrm{T}(t)$, which is filtered by the inductor. The semiconductor utilization is
            \begin{equation}
                \begin{split}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{equation}
                ...
                \end{split}
            \end{equation}
            The transistor must block the (combined) input and output voltage and step-like changing current $i_1(t)=i_\mathrm{T}(t)$. The semiconductor utilization is
            \begin{equation}
                \begin{split}
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
             \begin{itemize}
                \item The converters' semiconductor utilization is generally the highest if the input and output voltages are similar:
                \begin{itemize}
                    \item Step-down: $D\rightarrow1$,
                    \item Step-up: $D\rightarrow0$,
                    \item Inv. buck-boost: $D\rightarrow0.5$.
                \end{itemize}
                \item Inverting buck-boost has generally a lower utilization.
                \item Finding indicates that the inv. buck-boost  should be only considered if an application truly requires both step-up and step-down operation.
             \end{itemize}
        \end{column}
...
\begin{frame}[b]
    ...
    \end{itemize}
        ...
        \begin{itemize}
            ...
            \item Additional input capacitor necessary to buffer the pulsating input current.
...
\begin{frame}
    ...
    \end{equation}
    assuming that the input capacitor is loaded with the input current $I_1$ during the off-time $T_\mathrm{off}=(1-D)T_\mathrm{s}$. Assuming that there is an \hl{input voltage ripple requirement} on $\Delta u_\mathrm{C_1}$, that is, an upper limit ripple, the minimum input capacitance is 
    \begin{equation}
        C_1 \geq \frac{I_1(1-D)T_\mathrm{s}}{\Delta u_\mathrm{C_1}}=C_{1,\mathrm{min}}.
    \end{equation}
    The stored input capacitor energy yields
    \begin{equation}
        E_\mathrm{C_1} = \frac{1}{2} C_{1,\mathrm{min}} \left(U_1 + \frac{1}{2}\Delta u_\mathrm{C_1}\right)^2 = \frac{1}{2} (1-D)\frac{P_2}{f_\mathrm{s}}\frac{\left(1+\frac{\varepsilon_{u_\mathrm{C_1}}}{2}\right)^2}{\varepsilon_{u_\mathrm{C_1}}}
    \end{equation}
...
\begin{frame}
    \frametitle{Filter component requirements: step-down converter (cont.)}
    We already know from \eqref{eq:current-ripple-simple-step-down} the inductor current ripple being
    \begin{equation*}
    ...
    \begin{equation}
        E_\mathrm{L} = \frac{1}{2} L_{\mathrm{min}} \left(I_2 + \frac{1}{2}\Delta i_\mathrm{L}\right)^2 = \frac{1}{2} (1-D)\frac{P_2}{f_\mathrm{s}}\frac{\left(1+\frac{\varepsilon_{i_\mathrm{L}}}{2}\right)^2}{\varepsilon_{i_\mathrm{L}}}
    \end{equation}
...
\begin{frame}
    ...
    \begin{equation*}
        \Delta u_\mathrm{C_2} = \frac{D(1-D)T_\mathrm{s}^2U_1}{8LC_2} = \frac{(1-D)}{8LC_2}\frac{U_2}{f_\mathrm{s}^2}.
    \end{equation*}
    Inserting the inductor sizing \eqref{eq:inductor-minimum-inductance-step-down} delivers
    \begin{equation}
        \Delta u_\mathrm{C_2} = \frac{1}{8C_2} \frac{\Delta i_\mathrm{L}}{f_\mathrm{s}} = \frac{\varepsilon_{i_\mathrm{L}}}{8C_2} \frac{I_2}{f_\mathrm{s}}.
    \end{equation}
    ...
    \begin{equation}
        E_\mathrm{C_2} = \frac{1}{2} C_{2,\mathrm{min}} \left(U_2 + \frac{1}{2}\Delta u_\mathrm{C_2}\right)^2 = \frac{1}{16} \varepsilon_{i_\mathrm{L}}\frac{P_2}{f_\mathrm{s}}\frac{\left(1+\frac{\varepsilon_{u_\mathrm{C_2}}}{2}\right)^2}{\varepsilon_{u_\mathrm{C_2}}}.
    \end{equation}
\end{frame}
...
\begin{frame}
    ...
    The \hl{stored energy} in the filter components is a good proxy for the \hl{filter size and weight}. All three step-down converter filter components share the following characteristics:
    \begin{itemize}
        ...
        \end{itemize}
    \end{itemize}
    In addition, $E_\mathrm{L}$ and $E_\mathrm{C_1}$ also scale with $$(1-D),$$ that is, are small if the converter's input and output voltage are similar.    
\end{frame}
...
\begin{frame}
    ...
    We further assume an identical normalized input and output voltage ripple requirement 
    $$
    ...
    \begin{itemize}
        \item Same dependence on $P_2$, $f_\mathrm{s}$, and $\varepsilon_{u_\mathrm{C}}$ or $\varepsilon_{i_\mathrm{L}}$.
        \item Missing $(1-D)$ scaling factor.
        \item Result: The inverting buck-boost converter passive components are generally larger due to the pulsating input and output current which needs to be filtered.
    \end{itemize} 
\end{frame}
    ...
    \frametitle{Ćuk converter: the boost-buck converter with capacative energy transfer}
        \begin{figure}
            \begin{circuitikz}[]
                ...
                %Boost part
                \draw (-5,2) to [open, o-o, v = $u_1(t)\hspace{0.5cm}$, voltage = straight] ++(0,-2)
                to [short, -*]++(5,0)
                ...
                \draw (-2.75,2) to [open, v_>=$u_{\mathrm{s}_1}(t)\hspace{0.5cm}$, voltage = straight] ++(0,-2)
               ...
               \begin{scope}[on background layer]
                ...
                % Buck part
                \draw (0,0) to [short, o-]++(5,0)
                (5,2) to [open, o-o, v= $\hspace{1.9cm}u_2(t)$, voltage = straight] ++(0,-2)
                (0,2) [short] ++(1.0,0)
                to ++(0.1,0) node [cuteopenswitchshape, anchor = out, rotate=180] (S1) {}
                ...
                ([yshift = -0.3cm]S1.mid) to [short, o-*](\x1,0);
                \draw (2.75,2) to [open, v = $\hspace{1.75cm}u_\mathrm{s_2}(t)$, voltage = straight] ++(0,-2)
            ...
            \begin{circuitikz}[]
                ...
                to [open, o-o, v = $\hspace{1.9cm}u_2(t)$, voltage = straight] (S.cin -| C) -- (S.cin)
                (A) to [inductor, -o, i_<=$i_1(t)$, l_=$L_1$, mirror] ++(-3,0) coordinate (D)
                to [open, o-o, v = $u_1(t)\hspace{0.5cm}$, voltage = straight] (S.cin -| D) -- (S.cin)
                (S.out 1) node[below, anchor = north east] {$S_\mathrm{on}$}
                (S.out 2) node[below, anchor = north west] {$S_\mathrm{off}$};
            \end{circuitikz}
    ...
    \begin{figure}
        ...
        \begin{subfigure}{0.45\textwidth}
            ...
            \begin{circuitikz}[]
                ...
                to [open, o-o, v = $\hspace{1.5cm}U_2$, voltage = straight] ++(0,-2)
                to [short] (C);    
            \end{circuitikz}
            \caption{Switch-on time}
        \end{subfigure}%
...
\begin{frame}
    ...
    \begin{equation}
        L_1:\quad D U_1 + (1-D)(U_1-U_\mathrm{C})=0, \qquad L_2:\quad D(U_2+U_\mathrm{C}) + (1-D)U_2=0.
    \end{equation}
    Above, $U_1$, $U_2$, and $U_\mathrm{C}$ are considered constant. From those we can derive:
    \begin{equation}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Transistor $T$ needs to block $U_1$ during the off-time, while it covers both the input and output current during the on-time: semiconductor utilization is also $\nicefrac{P_2}{P_\mathrm{T}}=(1-D)D$ as for the inverting buck-boost (cf. \figref{fig:semiconductor-utilization-summary}).
    \end{itemize}
    \begin{figure}
        \begin{circuitikz}[]
            ...
            (B) to [diode, l=$D$, *-*] ++(0,-2)
            (B) to [inductor, -o, i=$i_2(t)$, l=$L_2$] ++(3,0) coordinate (C)
            to [open, o-o, v = $\hspace{1.9cm}u_2(t)$, voltage = straight] ++(0,-2)
            (A) to [inductor, -o, i_<=$i_1(t)$, l_=$L_1$, mirror] ++(-3,0) coordinate (D)
            to [open, o-o, v = $u_1(t)\hspace{0.5cm}$, voltage = straight] ++(0,-2)
            to [short, o-o] ++(9,0);
            \draw let \p1 = (npn1.B) in node[anchor=east] at (\x1,\y1) {$T$};
        \end{circuitikz}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Output current becomes pulsating (compared to Ćuk).
        \item Input to output gain becomes non-inverting (cf. next slides).
    \end{itemize}
    \begin{figure}
        \begin{circuitikz}[]
            ...
            to [open, o-o, v = $\hspace{1.9cm}u_2(t)$, voltage = straight] ++(0,-2.5)
            (A) to [inductor, -o, i_<=$i_1(t)$, l_=$L_1$, mirror] ++(-3,0) coordinate (D)
            to [open, o-o, v = $u_1(t)\hspace{0.5cm}$, voltage = straight] ++(0,-2.5)
            to [short, o-o] ++(9,0);
            \draw let \p1 = (npn1.B) in node[anchor=east] at (\x1,\y1) {$T$};
        \end{circuitikz}
...
\begin{frame}
    ...
    \begin{itemize}
        \item During switch on-time, the transistor is conducting and the diode is blocking (cause the output current pulsation).
        \item During switch off-time, the diode is conducting and the transistor is blocking.
    \end{itemize}
    \begin{figure}
        \centering	
        \begin{subfigure}{0.45\textwidth}
            ...
            \begin{circuitikz}[]
                ...
                to [open, o-o, v = $\hspace{1.5cm}U_2$, voltage = straight] ++(0,-2.5)
                to [short] ++(-0.75,0);    
            \end{circuitikz}
            \caption{Switch-on time}
        \end{subfigure}%
...
\begin{frame}
    ...
    In periodic steady-state operation, the voltage balance during a switching period of the two inductors must be fulfilled:
    \begin{equation}
        L_1:\quad D U_1 + (1-D)(U_1-U_2-U_\mathrm{C})=0, \qquad L_2:\quad -DU_\mathrm{C} + (1-D)U_2=0.
    \end{equation}
    Above, $U_1$, $U_2$, and $U_\mathrm{C}$ are considered constant. From those we can derive:
    \begin{equation}
        L_1:\quad U_\mathrm{C}=\frac{U_1}{1-D}-U_2, \qquad L_2:\quad U_\mathrm{C}=\left(\frac{1}{D}-1\right)\frac{U_2}{D}.
    \end{equation}
