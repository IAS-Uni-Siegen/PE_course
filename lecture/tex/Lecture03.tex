    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \begin{itemize}
                ...
                \item Different to flyback, where the transformer both stores energy and provides galvanic isolation.
            \end{itemize}
        \end{column}
        %
        \begin{column}{0.6\textwidth}
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \begin{varblock}{Magnetizing inductance}
                    With every switching cycle the primary magnetizing current $i_\mathrm{L_\mathrm{m}}(t)$ increases (transformer saturates and takes damage).
            \end{varblock}
            \begin{figure}
                \begin{tikzpicture}
                    \pgfmathsetmacro{\D}{0.6} % duty cycle
                    \begin{axis}[
                        ...
                        \node[signalred, above, inner sep = 2pt, anchor = south, fill=white] at (axis cs:0.6, 1.1) {$i_\mathrm{L_\mathrm{m}}(t)$};
                    \end{axis}
                \end{tikzpicture}
            \end{figure}
        \end{column}
