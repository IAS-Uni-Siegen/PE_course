...
\begin{frame}[c]
	...
	\begin{figure}
		% Note: Ensure \usepackage{pgfplots} and \usetikzlibrary{math} are in your preamble
		\begin{tikzpicture}
			\tikzmath{
				real \ac, \i2, \ic;
				\i2 = 1; % load current
...
\begin{frame}[c]
	...
	\end{equation}
	\onslide<2->{The \hl{total reactive power} is given by}
	\begin{equation}
		\onslide<2->{Q_1 = \sqrt{S_1^2 - P_1^2}} \onslide<3->{= S_1^{(1)}\sqrt{\frac{\pi^2}{8} - \cos^2(\alpha)}=\frac{\sqrt{2}}{\pi} \frac{N_2}{N_1} I_2U_1\sqrt{\frac{\pi^2}{8} - \cos^2(\alpha)}.}
	\end{equation}
	\onslide<4->{ Alternatively, one could also determine the harmonic reactive power}
	\begin{equation}
		\onslide<4->{Q_{1}^{(\mathrm{h})} = \sqrt{\left(S_1\vphantom{S_1^{(1)}}\right)^2 - \left(S_1^{(1)}\right)^2}} \onslide<5->{= S_1^{(1)}\sqrt{\frac{\pi^2-8}{8}}=\frac{\sqrt{2}}{\pi} \frac{N_2}{N_1} I_2U_1\sqrt{\frac{\pi^2-8}{8}}.}
	\end{equation}
	\onslide<6->{first and then determine the total reactive power as}
	\begin{equation*}
		\onslide<6->{Q_1 = \sqrt{\left(Q_1^{(1)}\right)^2 + \left(Q_{1}^{(\mathrm{h})}\right)^2 }} \onslide<7->{= S_1^{(1)}\sqrt{\sin^2(\alpha)+\frac{\pi^2-8}{8}}=\frac{\sqrt{2}}{\pi} \frac{N_2}{N_1} I_2U_1\sqrt{\sin^2(\alpha)+\frac{\pi^2-8}{8}}.}
	\end{equation*}
\end{frame}
