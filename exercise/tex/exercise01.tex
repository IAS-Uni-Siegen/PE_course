...
\input{fig/exec01/figCircuitWithOneTransistorAndOneLoadResistor}
...
\input{fig/exec01/sfigDisplayOfCurrentsAndVoltagesDependingOnTheSwitchingPhases.tex}
...
\input{fig/exec01/sfigInstantaneousPowerAtTheLoadResistor}
...
\input{fig/exec01/figStepDownConverterOutputFilter}
...
\input{fig/exec01/sfigSwitchingStatesStepDownConverter.tex }
...
\input{fig/exec01/sfigRelevantVoltageAndCurrentSignals}
...
The BCM operation is characterized by zero inductor current events at the beginning / end of a switching period, compare \autoref{fig:BCM_step-down-converter}.
    ...
    \input{fig/exec01/sfigQualitativeInductorCurrentDuringBCM}
...
\input{fig/exec01/figBuckConverterWithOneTransistorAndOneDiode}
...
\input{fig/exec01/figSwitchOnBehaviorAndSwitchOffBehaviorOfUI}
...
Finally, \autoref{fig:Power values for the switch-off process} results from these power values. The switch-off losses of the IGBT are calculated next. For this purpose, the power curve is divided into three parts:
    ...
    \input{fig/exec01/sfigSwitchOnBehaviorAndSwitchOffBehaviorOfPower}
