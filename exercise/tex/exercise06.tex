...
%solution
\begin{solutionblock}
    The maximum value of $u_\mathrm{1}(t)$ is calculated by of The voltage $u_\mathrm{2,p}(t)$ results from
    \begin{equation} 
        \hat{u}_\mathrm{1}=\sqrt{2} \cdot U_\mathrm{1}= \sqrt{2} \cdot \SI{230}{\volt}=\SI{325}{\volt}.
        \label{eq:ex06t2_u1n_max}
    \end{equation}
    The voltage $u_\mathrm{2}(t)$ is calculated by
    \begin{equation} 
        u_\mathrm{2}(t) = \hat{u}_\mathrm{1} \frac{\pi}{p}\sin(\frac{p}{\pi}) \cos(\alpha).
        \label{eq:ex06t2_u2_gen}
    \end{equation}
    Solving \eqref{eq:ex06t2_u2_gen} with respect to $\alpha$ results in
    \begin{equation} 
        \alpha = \arccos( \frac{u_\mathrm{2}(t) \cdot p}{\hat{u}_\mathrm{1}\pi \sin(\frac{p}{\pi})}) =
        \begin{cases}
...
\begin{itemize}
    ...
    \item The current $i_\mathrm{1a}(t)$ and it's fundamental $i_{1a}^{(1)}(t)$.
