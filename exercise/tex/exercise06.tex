...
and the fundamental apparent power $S^\mathrm{(1)}$. Represent $P$, $Q^\mathrm{(1)}$ and $S^\mathrm{(1)}$ in the complex plane.}

% Subtask5
\subtask{Calculate $S$, $P$ and $Q^\mathrm{(1)}$ and the fundamental component $g_=\frac{I^\mathrm{(1)}}{I}$ and the power factor $\lambda$ 
as a function of the effective values $U$, $I$, $I^\mathrm{(1)}$ and the phase shift $\Phi$ between $\hat{u}_\mathrm{(1)}^\mathrm{(1)}$ and 
$\hat{i}^\mathrm{(1)}$. Calculate the values for raising and lowing load.}