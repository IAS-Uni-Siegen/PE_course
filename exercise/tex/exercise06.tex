...
\begin{solutionblock}
    ...
    To simplify the calculation the the current signal $i_\mathrm{1a}(t)$ is shift $\frac{2 \cdot \pi}{3}\SI{}{\radian}$ to the left, 
    so that it is axisymmetric. Cause by the symmetry the integration is performed only from \SI{0}{\degree} to \SI{180}{\degree}:
    \begin{equation}
        \begin{split}
    ...
    The difference belongs only to the phase position relative to $U_\mathrm{1}(t)$ and only shifted.
...
\end{solutionblock}
% Subtask4
\subtask{Calculate the fundamental current ratio $g=\frac{I^\mathrm{(1)}}{I}$ and the power factor $\lambda$ 
