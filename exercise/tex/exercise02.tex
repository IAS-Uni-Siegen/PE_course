...
\begin{solutionblock}
    ...
    \end{equation}
    And with the values set, the result is as follows:
    \begin{equation}
...
\subtask{ Define a suitable inductance for the coil $L$, so that the boost converter is operating in boundary conduction mode (BCM) when supplying the minimum output power $P_\mathrm{out,min}$. Determine the maximal switch-off current $i_{\mathrm{T}}$ of the transistor $T$ for the rated output current $I_{\mathrm{2}} = \SI{2}{\ampere}$.}
\begin{solutionblock}
    Because no losses are assumed, $P_{\mathrm{1,min}}=P_{\mathrm{2,min}}$.
    The following equation is used for the average current $I_{\mathrm{1,min}}$ over a period to generate the minimum power
    \begin{equation}
    ...
    At time $T_{\mathrm{s,on}}$, the average current to provide the minimum power corresponds to twice the value of the $I_{\mathrm{1,min}}$
    \begin{equation}
    ...
    \end{equation}
    Simplifications can be assumed for the case without saturation
    \begin{equation}
    ...
    \begin{equation}
        L=\frac{\Delta t u_\mathrm{L} }{\Delta i_\mathrm{L}}= \frac{D U_\mathrm{1}}{f_{\mathrm{s}}2I_{\mathrm{1min}}} = \frac{0.8 \cdot \SI{12}{\volt}}{\SI{100}{\kilo\hertz\cdot 2\cdot \SI{1.667}{\ampere}}} = \SI{57.6}{\micro\henry}.
    \end{equation}
    The maximum current flows through the transistor at the end of the switch-on interval
    \begin{equation}
        I_\mathrm{T,max} = I_\mathrm{on} + \frac{\Delta I_\mathrm{L}}{2}=\SI{10}{\ampere}+\frac{\SI{1.667}{\ampere}}{2} = \SI{10.833}{\ampere}.
    \end{equation}
\end{solutionblock}
...
% Solution
    ...
    The result plot is displayed in \autoref{fig:VoltageAtInductorInCase1}. \\
    ...
    \end{equation}
    Again consider one period:
    \input{./fig/ex02/sFigTab_Case1a2}
    \input{./fig/ex02/sFigDia_InductorVoltageCase2}

    The result plot is displayed in \autoref{fig:VoltageAtInductorInCase2x}.
...
% Solution
    ...
    This leads to
    \begin{equation}
        P_\mathrm{loss}=P_\mathrm{2} U_\mathrm{F} \left(\frac{1}{U_\mathrm{1}} + \frac{1}{U_\mathrm{2}} \cdot \frac{D_2}{1-D_2}\right).
        \label{eq:ploss}
    \end{equation}
    To minimize the losses depending on duty cycle you consider $P_\mathrm{loss}$=$f(D_2)$ and derivate it.
    \begin{equation}
...
% Solution
    ...
    \begin{equation}
        ...
        D=\frac{U_\mathrm{2}}{U_\mathrm{1}+U_\mathrm{2}}.
    \end{equation}
    With equation \refeq{eq:ploss2} this leads now to
    \begin{equation}
