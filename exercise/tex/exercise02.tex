...
\subtask{ Define a suitable inductivity for the coil L, so that the boost converter is operating in continuous conduction mode (CCM), for the minimum output power given in the specification above. Determine the maximal switch-off current $i_{\mathrm{T}}$ of the transistor $\mathrm{T}_1$ for the rated output current $I_{\mathrm{2}} = \SI{2}{\ampere}$.}
\begin{solutionblock}
...
\subtask{From now on, the influences of the resistor $R_\mathrm{L}$ are considered. Derive the efficiency $\eta$ and voltage ratio $U_\mathrm{1}/U_\mathrm{2}$ of the boost converter, in dependence on the duty cycle $D$ and the resistance ratio $\alpha= R_\mathrm{L}/R$. Sketch both functions, $\eta$ and  $U_\mathrm{2}/U_\mathrm{1}$ over the duty cycle $D$ and give comments. Calculate $D$ under consideration of $R_\mathrm{L}=\SI{0.2}{\ohm}$.}
\begin{solutionblock}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subtask{In addition, consider the conduction losses of the diode D and the transistor T. Assume an equivalent resistance of $R_{\mathrm{D}} = \SI{0.5}{\milli\Omega}$ and voltage of $U_{\mathrm{D}} = \SI{1}{\volt}$ for the diode and an equivalent resistance of $R_{\mathrm{DS,on}} = \SI{30}{\milli\Omega}$ for the transistor. Determine the required duty cycle when the conduction losses are considered.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subtask{Beside the conduction losses, also switching losses need to be considered in reality. In \autoref{fig:TurnOffSiliconDiode} the voltage and current waveforms are visualized for the turn-off event of a diode. Therefore, calculate the turn-off losses for a fast diode with a commutation inductivity loop of $L_{\mathrm{c}} = \SI{500}{\nano\henry}$. The peak reverse recovery current is $\hat{i}_{\mathrm{rr}} = \SI{4}{\ampere}$ and the revere recovery time is $t_{\mathrm{rr}} = t_2 - t_0 = \SI{46.6}{\nano\second}$.}
...
\subtask{Calculate the relative duty cycle of the transistors $\mathrm{T_1}$ and $\mathrm{T_2}$ depending on the voltage transformation ratio $U_\mathrm{2}/U_\mathrm{1}$}
\subtask{Calculate $i_\mathrm{L}$ depending on $D$. Plot $D$ and $i_\mathrm{L}$ against $U_\mathrm{1}$ and enter the numerical values for $U_\mathrm{1} = \SI{320}{\volt}$, $U_\mathrm{1} = \SI{400}{\volt}$ and $U_\mathrm{1} = \SI{720}{\volt}$.}
\vspace{2em}\par
...
\subtask{Graphically represent the time profiles of the voltage at $L$ for $U_\mathrm{1}$ = $\SI{320}{\volt}$ and
 $D_1 = 0.9$ or for $U_\mathrm{1}$ = $\SI{720}{\volt}$ and $D_2 = 0.1$ for one pulse period each.}
...
\subtask{Are the calculated relationships valid if $\mathrm{T_1}$ and $\mathrm{T_2}$ do not switch synchronously or operate with
 different clock frequencies?}
\vspace{2em}\par
% Explaining text for the next subtask
If the transistors $\mathrm{T_1}$ and $\mathrm{T_2}$ are switched on, a constant voltage drop $U_\mathrm{F}=\SI{2.5}{\volt}$ occurs at transistors,
...
\subtask{Calculate the efficiency for the three operating points in subtask 3.2 and compare the numerical values.}
