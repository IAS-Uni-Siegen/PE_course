...
\begin{solutionblock}
	...
	The ripple current of $L_2$ is obtained by
	\begin{equation}
...
\begin{solutionblock}
	...
	\begin{equation}
		P_\mathrm{min}=U_\mathrm{2}\frac{\Delta i_\mathrm{L1}}{2}.
		\label{eq:PowerL1ripplesepic}
	\end{equation}
	\input{./fig/ex03/sFigTab_PowerminSepic} %Solution table Minimal Power 
	Entering the values in \eqref{eq:PowerL1ripplesepic} leads to \autoref{table:PowerminSepic}. Similar to the boost-buck converter the minimal power,
	which ensures continuous operation across the entire output voltage range yields $\SI{1854}{\watt}$.

\end{solutionblock}

\subtask{Describe the advantage and disadvantage of the SEPIC topology
	compared to the boost-buck converter. Consider the necessary components
	and the quality of the output voltage.}

\begin{solutionblock}
	% Solution
	Advantage of SEPIC-topology compared to boost-buck converter:
	\begin{itemize}
		\item The amount of semiconductor components are reduced (only 1 transistor and 1 diode is needed).
		\item Lower voltage rating requirement for the capacitor ($U_1$ compared to $U_0$).
	\end{itemize}
	Disadvantage of SEPIC-topology compared to boost-buck converter:
	\begin{itemize}
