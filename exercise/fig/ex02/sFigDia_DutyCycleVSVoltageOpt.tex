%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Dutycycle versus Input voltage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{solutionfigure}[htb]
    \centering
    \begin{tikzpicture}
        \begin{axis}[,
                % x/y range adjustment
                xmin=300, xmax=820,
                ymin=0, ymax=1.1,
                samples=500,
                axis y line=center,
                axis x line=middle,
                extra y ticks=0,
                % Label text
                xlabel={$U_\mathrm{1} \text{/V}$},
                ylabel={$D(U_\mathrm{1})$},
                % Label adjustment
                x label style={at={(axis description cs:1,0)},anchor=west},
                y label style={at={(axis description cs:-.05,1)},anchor=south},
                width=0.6\textwidth,
                height=0.3\textwidth,
                % x-Ticks                
                xtick={300,400,500,600,700,800},
                xticklabels={300,400,500,600,700,800},
                xticklabel style = {anchor=north},
                % y-Ticks                
                ytick={0,0.2,0.4,0.6,0.8,1},
                yticklabels={0,0.2,0.4,0.6,0.8,1},
                yticklabel style = {anchor=east},
                % Grid layout                
                grid=both,
                grid style={line width=.1pt, draw=gray!10},
                major grid style={line width=.2pt,draw=gray!50},
            ]
            % D1-plot
            \addplot[color=red,mark=none,solid] coordinates{
             (320, 1)
             (400, 1)
            };
            \addplot[signalred, domain=400:720] {400/x};
            % Label of D1
            \node[signalred, fill=white, inner sep = 1pt, anchor = south] at (axis cs:550,0.8) {$D_{\mathrm{1}}$};
            % D2-plot
            \addplot[color=blue,mark=none,solid] coordinates{
             (400, 0)
             (720, 0)
            };
            \addplot[signalblue, domain=320:400] {1-(x/400)};
            % Label of D2
            \node[signalblue, fill=white, inner sep = 1pt, anchor = south] at (axis cs:350,0.2) {$D_{\mathrm{2}}$};
        \end{axis}
    \end{tikzpicture}
    \caption{Duty cycle versus input voltage.}
\end{solutionfigure}






