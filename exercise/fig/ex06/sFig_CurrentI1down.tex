...
\begin{solutionfigure}[htb]
 ...
 %   \pgfplotsset{compat=1.18} % Kompatibilität für neuere Versionen
        ...
        \begin{tikzpicture}
            ...
            % Label of i1a
            \node[red, fill=white, inner sep = 1pt, anchor = south] at (axis cs:260,20) {$i_{\mathrm{1a}}(\omega t)$};
            % Line to i1a
            \draw[thin, red] (218,21) -- (233,24); 
            % Label of i1afundamental
            \node[black, fill=white, inner sep = 1pt, anchor = south] at (axis cs:45,10) {$i_{\mathrm{1afund}}(\omega t)$};
            % Line to u1a
            \draw[thin, black] (82,14) -- (90,12);                
        \end{axis}     
        \end{tikzpicture}
        \caption{Output current $i_\mathrm{1a}(t)$ and it's fundamental amplitude  for lowing load.}
