...
The dynamo supplies a variable voltage. A control circuit adjusts the converter's duty cycle 
...
\begin{germanblock}
    ...
    Der Dynamo liefert eine variable Spannung. Eine Reglungsschaltung steuert das Tastverhältnis des Konverters,
...
\subtask{What are the minimum and maximum duty cycles that the control circuit has to request in case of $D = D_1 = D_2$?}{3}
...
\begin{solutionblock}
    ...
    \begin{align*}
        D_\mathrm{max} &= \frac{\SI{5}{\volt}+ 2 \cdot \SI{0.8}{\volt}}{\SI{3}{\volt} - 2 \cdot \SI{0.5}{\volt}+\SI{0.8}{\volt + \SI{5}{\volt}+ 2 \cdot \SI{0.8}{\volt}}}  
                        = 0.77 \\
        D_\mathrm{min} &= \frac{\SI{5}{\volt}+ 2 \cdot \SI{0.8}{\volt}}{\SI{10}{\volt} - 2 \cdot \SI{0.5}{\volt}+\SI{0.8}{\volt + \SI{5}{\volt}+ 2 \cdot \SI{0.8}{\volt}}}  
                        = 0.42
    \end{align*}
\end{solutionblock}
...
\subtask{Calculate the efficiency of the converter at the operation points $U_\mathrm{1}=\SI{3}{\volt}$, $U_\mathrm{1}=\SI{7.5}{\volt}$, and $U_\mathrm{1}=\SI{10}{\volt}$
         at maximum output power. Consider only the losses cause be the constant voltage drop of the semiconductors. All other losses must me ignored.}{3}
\subtaskGerman{Berechne den Wirkungsgrand des Konverters an den Arbeitspunkten $U_\mathrm{1}=\SI{3}{\volt}$, $U_\mathrm{1}=\SI{7.5}{\volt}$, und $U_\mathrm{1}=\SI{10}{\volt}$
               bei maximaler Ausgangsleistung. Betrachten Sie nur die Verluste durch die Spannungsabfälle über die Halbleiter. Alle anderen Verluste sind zu vernachlässigen.}
...
\begin{solutionblock}
    ...
    \begin{equation*}
        \eta = \frac{P_\mathrm{2}}{U_\mathrm{2}+P_\mathrm{loss}} = \frac{U_\mathrm{1} I_\mathrm{2}} {U_\mathrm{1} I_\mathrm{2}+P_\mathrm{loss}}
    \end{equation*}
    For the operation points we obtain
    \begin{align*}
        \eta_\mathrm{U_\mathrm{1}=\SI{3}{\volt}} = \frac{\SI{0.5}{\volt} \cdot \SI{1}{\ampere}} {\SI{0.5}{\volt} \cdot \SI{1}{\ampere} + \SI{4.9}{\watt}} = 0.51\\
        \eta_\mathrm{U_\mathrm{1}=\SI{7.5}{\volt}} = \frac{\SI{0.5}{\volt} \cdot \SI{1}{\ampere}} {\SI{0.5}{\volt} \cdot \SI{1}{\ampere} + \SI{4.9}{\watt}} = 0.66\\
        \eta_\mathrm{U_\mathrm{1}=\SI{10}{\volt}} = \frac{\SI{0.5}{\volt} \cdot \SI{1}{\ampere}} {\SI{0.5}{\volt} \cdot \SI{1}{\ampere} + \SI{4.9}{\watt}} = 0.68\\
    \end{align*}
\end{solutionblock}
...
\subtask{To avoid operation in discontinuous conduction mode (DCM) within the defined operating range, the minimum inductance  $L$ must be calculated.
         Evaluate the general equation for boundary conduction mode (BCM) to indicate, at which operation point the discontinuous conduction mode occurs?
         Calculate the minimum required inductance $L{\mathrm{1}}$ for boundary conduction mode (BCM) at this operation point.
         $D = D_1 = D_2$ is still valid.}{3}
\subtaskGerman{ Um den Betrieb im diskontinuierlichen Leitungsmodus (DCM) innerhalb des definierten Arbeitsbereichs zu vermeiden, soll die minimale Induktivität 
                $L$ berechnet werden.\\
                Werten Sie die allgemeine Gleichung für den boundary conduction mode (BCM) her, um den Arbeitspunkt zu bestimmen, an dem der Übergang in den discontinuous conduction mode (DCM) auftritt.
                Berechnen Sie für diesen Arbeitspunkt die minimal erforderliche Induktivität für den boundary conduction mode (BCM). $D = D_1 = D_2$ ist noch gültig.}
...
\begin{solutionblock}
    ...
    \end{equation*}
    The term $(1-D_\mathrm)^2$ express, that at minimum duty cycle the highest inductance is needed, to prevent discontinuous conduction mode.
    Therefore $D_\mathrm{min}$ is used.
    \begin{equation*}
        L = \frac{(\SI{5}{\volt}+2 \cdot \SI{0.8}{\volt}) \cdot (1-0.42)^2}{2 \SI{50}{\milli\ampere} \cdot \SI{100}{\kilo\hertz}} 
         = \SI{219}{\micro\henry}
    \end{equation*}

\end{solutionblock}

\subtask{ Now the control circuit regulates the transistors independent. It means, for low input voltage he boost mode is used and for high input voltage the buck mode is uses.
...
\begin{solutionblock}
    ...
    \begin{equation*}
        \frac{(U_\mathrm{1}-2u_\mathrm{ds,on}) \cdot T_\mathrm{s} \cdot D}{L} =
        \frac{(U_\mathrm{2}+ u_\mathrm{fw} + u_\mathrm{ds,on}-U_\mathrm{1}) \cdot (1-D) \cdot T_\mathrm{s}}{L}
    \end{equation*}
    After simplifying by canceling $T_\mathrm{s}$ and $L$ we obtain
    \begin{equation*}
        (U_\mathrm{1}-2u_\mathrm{ds,on}) \cdot D =
        (U_\mathrm{2}+ u_\mathrm{fw} + u_\mathrm{ds,on}-U_\mathrm{1}) \cdot (1-D)
    \end{equation*}
    Solving this equation with respect to $D$ yields
    \begin{equation*}
        D = \frac{U_\mathrm{2}+ u_\mathrm{fw} + u_\mathrm{ds,on}-U_\mathrm{1}}{U_\mathrm{2}+ u_\mathrm{fw} - u_\mathrm{ds,on}}
    \end{equation*}
    For the operation point $U_\mathrm{1}=\SI{3}{\volt}$ we obtain
    \begin{equation*}
        D_\mathrm{U_\mathrm{1}=\SI{3}{\volt}}=\frac{\SI{5}{\volt}+ \SI{0.8}{\volt} + \SI{0.5}{\volt}-\SI{3}{\volt}}{\SI{5}{\volt} + \SI{0.8}{\volt} - \SI{0.5}{\volt}}
        = 0.52
    \end{equation*}
    ...
    Replacing $T_\mathrm{on}$ and $T_\mathrm{off}$ by the switching period yields to
    \begin{equation*}
        \frac{(U_\mathrm{1}-u_\mathrm{ds,on} - u_\mathrm{fw} - U_\mathrm{2}) \cdot T_\mathrm{s} \cdot D}{L} =
        \frac{(U_\mathrm{2} + 2 u_\mathrm{fw}) \cdot (1-D) \cdot T_\mathrm{s}}{L}
    \end{equation*}
    After simplifying by canceling $T_\mathrm{s}$ and $L$, we obtain
    \begin{equation*}
        (U_\mathrm{1}-u_\mathrm{ds,on} - u_\mathrm{fw} - U_\mathrm{2}) \cdot D =
        (U_\mathrm{2} + 2 u_\mathrm{fw}) \cdot (1-D)
    \end{equation*}
    Solving this equation with respect to $D$ yields
    \begin{equation*}
        D = \frac{U_\mathrm{2}+ 2 u_\mathrm{fw}}{U_\mathrm{1}-u_\mathrm{ds,on} + u_\mathrm{fw}}
    \end{equation*}
    For the operation points $U_\mathrm{1}=\SI{7.5}{\volt}$ and $U_\mathrm{1}=\SI{10}{\volt}$ we obtain
    \begin{align*}
        D_\mathrm{U_\mathrm{1} = \SI{7.5}{\volt}}&=\frac{\SI{5}{\volt} + 2 \cdot \SI{0.8}{\volt}}{\SI{7.5}{\volt} - \SI{0.5}{\volt} + \SI{0.8}{\volt}} = 0.85 \\
        D_\mathrm{U_\mathrm{1} = \SI{10}{\volt}}&=\frac{\SI{5}{\volt} + 2 \cdot \SI{0.8}{\volt}} {\SI{10}{\volt} - \SI{0.5}{\volt} + \SI{0.8}{\volt}} = 0.64
    \end{align*}
